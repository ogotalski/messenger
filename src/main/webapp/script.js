...
messageBlock.setReaded = function(user){
	...
	for (var i = 0; i < length; i++) {
		...
		if (user === userElem.getAttribute("data-user")) {
			 var el = userElem.querySelector(".newMessages");
			 el.innerHTML = 0;
...
messageBlock.addUser = function(user) {
	...
	el.innerHTML = user;
	messageBlock.USERS_LIST.NEWPOSTBOXES[user]=elem.querySelector(".newMessages");
...
messageBlock.incNewMessages= function (user){
	var el = messageBlock.USERS_LIST.NEWPOSTBOXES[user];
	if (el){
...
messageBlock.addMess = function(mess,readed) {
...
messageService.readMessage = function(message) {
	...
	if (this.currentUser && this.currentUser === user) {
		messageBlock.addMess(message, message.readed);
		if (!message.readed && message.sender != messageService.loginedUser){
			messageService.setReaded(message.sender);
		}
	}
	if (this.messages[user]) {
		
		this.messages[user].push(message);
		messageService.isNewMessage(message);
...
messageService.setReaded = function (user){
	 utils.setTimeOut(messageBlock.setReaded,1800 , user);
};
messageService.isNewMessage = function(message){
	if (!message.readed && message.sender != messageService.loginedUser){
		messageBlock.incNewMessages(message.sender);
	}
};
...
messageService.start = function() {
	...
	eventSource.addEventListener("message", function(ev) {
		...
		if (data.message) {
			messageService.readMessagesArray(data.message);
		} else {
...
messageService.viewUser = function(user) {
	...
	for ( var i in messageService.messages[user]) {
		mess = messageService.messages[user][i];
		messageBlock.addMess(mess,mess.readed);
		mess.readed = true;
		
		
	}
	;
	messageService.setReaded(user);
};
